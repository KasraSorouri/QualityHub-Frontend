...
const RecipeList = ({ recipes, displayRecipeForm, selectRecipe }: RecipeListProps) => {
  ...
  // Sort Items
  const [sort, setSort] = useState<{ sortItem: keyof Recipe; sortOrder: number }>({
    sortItem: 'recipeCode',
    sortOrder: 1,
  });
              ...
              return (
                <React.Fragment key={recipe.id}>
                  <TableRow hover role="checkbox" tabIndex={-1} key={recipe.id}>
                    ...
                    <TableCell align="center" sx={{ borderRight: '1px solid gray' }}>
                      <Button
                        onClick={() => setShowMaterials({ index, show: !showMatrials.show })}
                        variant="contained"
                        color="primary"
                      >
                    ...
                    <TableCell align="center">
                      <Box justifyContent={'space-between'}>
                        ...
                        <IconButton
                          onClick={() => showEditRecipe(recipe.id)}
                          title="Edit"
                  ...
                  <TableRow key={index}>
                    <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
                      <Collapse in={showMatrials.show && showMatrials.index === index} timeout="auto" unmountOnExit>
                        <Box margin={1}>
                          <RecipeBOM
                            bom={recipe.recipeMaterials ? recipe.recipeMaterials : []}
                            updateBOM={() => null}
                            readonly={true}
                          />
                        </Box>
