...
const Stations = () => {
  const [showStationForm, setShowStationForm] = useState<{ show: boolean; formType: 'ADD' | 'EDIT' }>({
    show: false,
    formType: 'ADD',
  });
  ...
  return (
    <Grid container direction={'column'} spacing={2}>
      <Grid item>
        ...
        {showStationForm.show && (
          <StationForm
            stationData={selectedStation}
            formType={showStationForm.formType}
            submitHandler={handleStationFormSubmit}
            displayStationForm={setShowStationForm}
          />
        )}
      </Grid>
      <Grid item>
        {stationResults.data && (
          <StationList stations={stations} selectStation={setSelectedStation} displayStationForm={setShowStationForm} />
        )}
      </Grid>
    </Grid>
  );
