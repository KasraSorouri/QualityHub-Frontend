...
const Filter_Analysed_NOK = ({ closeFilter, applyFilter }: FilterProps) => {
  const [filterParams, setFilterParams] = useState(
    window.sessionStorage.getItem('NokAnalysedFilter')
      ? JSON.parse(window.sessionStorage.getItem('NokAnalysedFilter') || '{}')
      : {},
  );

  const shiftList = useQuery('workShifts', workShiftServices.getShift, { refetchOnWindowFocus: false, retry: 1 });

  const productList = useQuery('productList', productServices.getProduct, { refetchOnWindowFocus: false, retry: 1 });
    ...
    <Dialog open={true}>
      ...
      <DialogActions>
        <Stack direction={'row'} spacing={2} justifyContent={'flex-end'}>
          <Button variant="outlined" size="small" onClick={() => closeFilter()}>
            Cancel
          </Button>
          <Button variant="outlined" size="small" onClick={() => clearFilter()}>
            Clear filter
          </Button>
          <Button variant="outlined" size="small" onClick={() => saveFilter()}>
            Set Filter
          </Button>
        </Stack>
      </DialogActions>
