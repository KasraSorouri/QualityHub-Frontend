...
const NokDismantledMaterialForm = ({
  affectedMaterials,
  nokDismantledMaterials,
  confirmSelection,
  confirmChange,
  editable,
  ...
  // Sort Items
  const [sort, setSort] = useState<{ sortItem: keyof FormDismantledMaterialData; sortOrder: number }>({
    sortItem: 'index2',
    sortOrder: 1,
  });
  ...
  const handleSelect = (_event: React.MouseEvent<unknown>, index: string) => {
    ...
    if (selectedMaterials.includes(selectedIndex)) {
      ...
      if (updateValue) {
        ...
        updateValue.actualDismantledQty = updateValue.suggestedDismantledQty
          ? updateValue.suggestedDismantledQty
          : updateValue.recipeQty;
      }
      setSelectedMaterials(newSelected);
    }
  ...
  const handleConfirmSelection = () => {
    ...
    dismantledmaterials.forEach((material) => {
      if (material.actualDismantledQty === 0) {
        material.actualDismantledQty = material.suggestedDismantledQty
          ? material.suggestedDismantledQty
          : material.recipeQty;
      }
  ...
  return (
    <Paper sx={{ pointerEvents: editable ? 'all' : 'none' }}>
      <Grid container bgcolor={'#1976d2d9'} color={'white'} justifyContent={'space-between'} flexDirection={'row'}>
        ...
        <Stack direction={'row'} spacing={1} margin={0.5}>
          ...
          <Button
            ...
            sx={{ height: '30px' }}
            onClick={selectAll}
            disabled={formValues.length === selectedMaterials.length}
          >
            Select All
          </Button>
          <Button
        ...
        <Table stickyHeader aria-label="sticky table" size="small" sx={{ tableLayout: 'auto' }}>
          <EnhancedTableHead
            ...
            onRequestSort={(_event, property) =>
              handleRequestSort(_event, property as keyof FormDismantledMaterialData)
            }
          />
          <TableBody>
                  ...
                  <TableRow key={eMaterial.index2}>
                    ...
                    <TableCell
                      ...
                      sx={{
                        ...
                        background: eMaterial.material
                          ? eMaterial.material.traceable
                            ? '#FCFEA0'
                            : '#A0F1FE'
                          : null,
                      }}
                    ...
                    <TableCell align="center" sx={{ borderRight: '1px solid gray' }}>
                      <TextField
                        ...
                        sx={{
                          '& .MuiInputBase-input': { maxHeight: '10px', padding: '8px', textAlign: 'center' },
                          overflow: 'hidden',
                        }}
                        variant="filled"
                    ...
                    <TableCell align="center" sx={{ borderRight: '1px solid gray' }}>
                      <TextField
                        ...
                        sx={{
                          ...
                          backgroundColor:
                              eMaterial.materialStatus === MaterialStatus.OK
                                ? '#A8F285'
                                : eMaterial.materialStatus === MaterialStatus.IQC
                                  ? '#FFFFAB'
                                  : eMaterial.materialStatus === MaterialStatus.CLAIMABLE
                                    ? '#44bafc'
                                    : '#f93a00',
                          overflow: 'hidden',
                          marginLeft: '-10px',
                        ...
                        onChange={(event) =>
                          handleExtraMatrialStatus(event.target.value as MaterialStatus, eMaterial.index2)
                        }
                      >
                  ...
                  <TableCell align="center" sx={{ borderRight: '1px solid gray' }}>
                    {material.isSelected ? (
                      <TextField
                        name="actualDismantledQty"
                        sx={{
                          '& .MuiInputBase-input': { maxHeight: '15px', padding: '5px', textAlign: 'center' },
                          overflow: 'hidden',
                        }}
                        variant="filled"
                          ...
                          backgroundColor:
                            material.materialStatus === MaterialStatus.OK
                              ...
                              : material.materialStatus === MaterialStatus.IQC
                                ? '#FFFFAB'
                                : material.materialStatus === MaterialStatus.CLAIMABLE
                                  ? '#44bafc'
                                  : '#f93a00',
                          overflow: 'hidden',
                          marginLeft: '-10px',
      ...
      >
        <DialogTitle id="alert-dialog-title">{'Removed Qty is greater than BOM Qty?'}</DialogTitle>
        <DialogContent>
