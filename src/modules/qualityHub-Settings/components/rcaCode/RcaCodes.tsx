...
const RcaCodes = () => {
  const [showRcaCodeForm, setShowRcaCodeForm] = useState<{ show: boolean; formType: 'ADD' | 'EDIT' }>({
    show: false,
    formType: 'ADD',
  });
  ...
  return (
    <Grid container direction={'column'} spacing={2}>
      <Grid item>
        ...
        {showRcaCodeForm.show && (
          <RcaCodeForm
            rcaCodeData={selectedRcaCode}
            formType={showRcaCodeForm.formType}
            submitHandler={handleRcaCodeFormSubmit}
            displayRcaCodeForm={setShowRcaCodeForm}
          />
        )}
      </Grid>
      <Grid item>
        {rcaCodeResults.data && (
          <RcaCodeList rcaCodes={rcaCodes} selectRcaCode={setSelectedRcaCode} displayRcaCodeForm={setShowRcaCodeForm} />
        )}
      </Grid>
    </Grid>
  );
