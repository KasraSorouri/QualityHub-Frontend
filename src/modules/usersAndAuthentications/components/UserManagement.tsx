...
import { FilterParamData, NewRight, NewRole, NewUser, Right, Role, RoleUpdate, UserBase, UserUpdate } from '../../../types/UserAuthTypes';
...
const UserManagement = () => {
  ...
  const [ showUserForm, setShowUserForm ] = useState<{ show: boolean, formType: 'ADD' | 'EDIT' }>({ show: false, formType: 'ADD' });
  const [ showRoleForm, setShowRoleForm ] = useState<{ show: boolean, formType: 'ADD' | 'EDIT' }>({ show: false, formType: 'ADD' });
  const [ showRightForm, setShowRightForm ] = useState<{ show: boolean, formType: 'ADD' | 'EDIT' }>({ show: false, formType: 'ADD' });
          ...
          <Paper >
            ...
            <Grid container spacing={1} height={'600px'}>
              <Grid item xs={7}>
                ...
                { filteredUsers && <UsersList users={filteredUsers} allUsers={userResults.data ? userResults.data?.length : 0} displayUserForm={setShowUserForm} selectUser={setSelectedUser} />}
              </Grid>
              <Grid item xs={5}>
                <Stack direction={'column'}>
                  <Box height={'50%'} maxHeight={'300px'}>
                    { roleResults.isLoading && <LinearProgress sx={{ margin: 1 }}/> }
                    { filteredRoles && <RoleList roles={filteredRoles} allRoles={roleResults.data ? roleResults.data.length : 0} displayRoleForm={setShowRoleForm} selectRole={setSelectedRole}/>}
                  </Box>
                  <Box height={'50%'} maxHeight={'300px'} marginTop={1}>
                    { rightResults.isLoading && <LinearProgress sx={{ margin: 1 }}/> }
                    { filteredRights && <RightList rights={filteredRights} allRights={rightResults.data ? rightResults.data.length : 0} displayRightForm={setShowRightForm} selectRight={setSelectedRight}/>}
                  </Box>
                </Stack>
              </Grid>
            </Grid>
