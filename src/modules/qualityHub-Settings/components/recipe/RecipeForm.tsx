...
const RecipeForm = ({ recipeData, productId, formType, submitHandler, displayRecipeForm }: RecipeFormProps) => {
  ...
  const handleMaterialChange = (newValue: ConsumingMaterial[]) => {
    const newMaterials = newValue.map((item) => {
      return {
        id: item.id,
  ...
  const handleSubmit = (event: { preventDefault: () => void }) => {
    ...
    const materialsData = formValues.materials
      ?.map((item) => {
        if (!item.material) {
          return undefined;
        } else {
        ...
        <Box display="flex" margin={0}>
          <Grid container flexDirection={'column'}>
            ...
            <Grid container flexDirection={'row'}>
              ...
              >
                <ToggleButton value="PRODUCTION" sx={{ '&.Mui-selected': { backgroundColor: '#96FFD9' } }}>
                  Production
                </ToggleButton>
                <ToggleButton value="REWORK" sx={{ '&.Mui-selected': { backgroundColor: '#56F0FA' } }}>
                  Rework
                </ToggleButton>
              </ToggleButtonGroup>
              <FormControlLabel
            ...
            <Grid margin={2}>
              {showMaterials && (
                <RecipeBOM
                  bom={formValues.materials ? formValues.materials : []}
                  updateBOM={handleMaterialChange}
                  readonly={false}
                />
              )}
            </Grid>
          ...
          <Grid item xs={2} margin={1}>
            ...
            <Button
              variant="contained"
              color="primary"
              sx={{ marginTop: 2, minWidth: '200px', maxHeight: 40, width: 'auto' }}
