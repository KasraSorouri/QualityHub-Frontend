import { Box, FormControlLabel, Radio, RadioGroup, Stack, Typography, Checkbox } from '@mui/material';
    ...
    <Box sx={{ display: 'flex', width: 300, border: 1, borderColor: 'grey.500', borderRadius: 1, paddingLeft: 1 }}>
      <Stack direction="column">
        ...
        >
          ...
          <FormControlLabel
            value={NokStatus.ANALYSED}
            checked={status.analyseStatus === 'ANALYSED'}
            disabled={!closeAnalysePermision}
            control={<Radio />}
            label="Analyse is done"
          />
          <FormControlLabel
            value={NokStatus.NEED_INVESTIGATION}
            checked={status.analyseStatus === 'NEED INVESTIGATION'}
            control={<Radio />}
            label="More investigation should be done"
            sx={{ marginTop: -1 }}
          />
          <FormControlLabel
            value={NokStatus.NOT_FOUND}
            checked={status.analyseStatus === 'NOT FOUND'}
            control={<Radio />}
            label="Nok did not found"
            sx={{ marginTop: -1 }}
          />
        </RadioGroup>
        <FormControlLabel
