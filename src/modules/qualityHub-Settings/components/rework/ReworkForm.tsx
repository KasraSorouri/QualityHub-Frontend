...
const ReworkForm = ({ reworkData, formType, product, displayReworkForm, updateRequest }: ReworkFromProps) => {
  ...
  if (formType === 'ADD') {
    disableSubmmit =
      !formValues.reworkShortDesc ||
      !formValues.nokCode ||
      !formValues.station ||
      !confirmation.affectedRecipes ||
      !confirmation.reworkRecipes ||
      !confirmation.dismantledMaterials;
  } else {
    if (formType === 'EDIT') {
    ...
    <Grid container direction={'column'}>
      <Box sx={{ height: '600px', overflowY: 'scroll', paddingRight: '10px' }}>
        <form onSubmit={handleSubmit}>
            ...
            <Grid item xs={1}>
              <Stack direction={'column'}>
                ...
                <Button
                  ...
                  sx={{ margin: 1, marginLeft: 1, width: 'auto', height: '38px' }}
                  onClick={() => {
                    displayReworkForm({ show: false, formType: 'ADD' });
                  }}
                >
                  Back
                </Button>
          ...
          <Divider sx={{ margin: 1 }} />
          <ReworkRecipeList
            recipes={reworkRecipes}
            selectedRecipes={reworkData?.reworkRecipes ? reworkData.reworkRecipes : []}
          ...
          <ReworkRecipeList
            ...
            selectedRecipes={reworkData?.affectedRecipes ? reworkData.affectedRecipes : []}
